...
// 메일 내용을 생성하고 임시 비밀번호로 회원 비밀번호를 변경
    ...
    public List<StudyMembershipStatusResponseDto> getStudyMembershipStatus(String userEmail) throws NotFoundException {
        ...
        List<StudyMemberRoomEntity> userStudyMemberRooms = studyMemberRoomRepository.findAllByMember_MemberId(memberId);

        List<StudyMembershipStatusResponseDto> membershipStatusList = userStudyMemberRooms.stream()
                .map(memberRoom -> {
                    StRoomEntity stRoom = memberRoom.getRoom();
                    String title = stRoom.getTitle();
                    String status = memberRoom.getStatus().name();
                    return new StudyMembershipStatusResponseDto(title, status);
                })
